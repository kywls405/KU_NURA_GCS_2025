...
  let isConnected = false;
  let launchSignalReceived = false; // [개선] 발사 신호 수신 여부 플래그

  const socket = io();

  // --- 함수 정의 ---

  // [개선] Event Log에 상태 표시등(색깔 원)을 추가하는 기능 구현
...
    apogeeLogged = false;
    isConnected = false;
    launchSignalReceived = false; // [개선] 플래그 초기화
...
    // // 비정상적으로 높은 고도 값(오버플로우 의심)이 들어오면 0으로 처리
    // if (data.Alt > 400) { data.Alt = 0; }
    // if (data.P_alt > 400) { data.P_alt = 0; }
    
...
    updateLaunchStatus(data.launch);
    ...
    // [개선] 발사 신호(launch === 1)가 처음 수신되면, 발사 전 caution 상태를 초기화
    if (data.launch === 1 && !launchSignalReceived) {
        launchSignalReceived = true;
        clientEjectionCautionStatus = 0; // SAFE 상태로 강제 초기화
    }

    if (data.ejection > 0 && !isEjected && data.launch === 1) {
      isEjected = true;
      updateEjectionStatus(data.ejection); // 서버에서 받은 사출 상태로 업데이트
...
    } else if (!isEjected) {
        // [개선] 비행 시작 후에만 caution 로직이 동작하도록 개선
        if (data.flight_timestamp > 0) {
    ...
            const alt_caution = altNum > 350;
            const time_caution = flightSeconds > 9;

            // [개선] 순차적으로 caution을 검사하고 가장 먼저 발생한 것을 표시 (if -> if-else if)
            if (clientEjectionCautionStatus === 0) {
                if (angle_caution) clientEjectionCautionStatus = 1;
                else if (alt_caution) clientEjectionCautionStatus = 2;
                else if (time_caution) clientEjectionCautionStatus = 3;
            }
        }
        updateEjectionStatus(clientEjectionCautionStatus); // 현재 caution 상태(또는 초기화된 SAFE)로 UI 업데이트
    }
    
...
    addLogMessage(connectMessage, 'info', true); // [개선] 연결 시도 로그에 표시등 추가
    
